...
def start(plan, node, service, genesis_artifact, hash_sig_artifact):
        ...
        plan.exec(
            ...
            recipe=ExecRecipe(
                command=[
                    "/bin/sh",
                    "-c",
                    "cat > {0}/{1} <<'LIGHTHOUSE_EOF'\n{2}\nLIGHTHOUSE_EOF".format(
                        GENESIS_MOUNT,
                        filename,
                        read.output,
                    ),
    ...
    cmd_parts = [
        ...
        "--genesis-json",
        "{0}/genesis.json".format(GENESIS_MOUNT),
        "--socket-port",
    ...
    plan.exec(
        ...
        description="Starting lighthouse binary on {0}".format(service_name),
    )
    ...
    return lean_context.new_lean_context(
        client_name=constants.LEAN_TYPE.lighthouse,
        service_name=service_name,

...
Translates the Lean pipeline's per-node record into qlean's CLI surface
from `client-cmds/qlean-cmd.sh` in blockblaz/lean-quickstart:

    qlean \
      --genesis-dir <dir> \
      --data-dir <dir> \
      --node-id <name> --node-key <key> \
      --listen-addr /ip4/0.0.0.0/udp/<quic>/quic-v1 \
      --metrics-host 0.0.0.0 --metrics-port <m> \
      --api-host 0.0.0.0 --api-port <api>
"""
...
def start(plan, node, service, genesis_artifact, hash_sig_artifact):
    ...
    plan.exec(
        ...
        description="Starting qlean binary on {0}".format(service_name),
    )
    ...
    return lean_context.new_lean_context(
        client_name=constants.LEAN_TYPE.qlean,
        service_name=service_name,
